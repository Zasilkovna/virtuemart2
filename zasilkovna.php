...
class plgVmShipmentZasilkovna extends vmPSPlugin
{
  ...
  function calculateSalesPrice ($cart, $method, $cart_prices) {    
    ...
    function getCosts(VirtueMartCart $cart, $method, $cart_prices){                   
        ...
        $pricesCurrencyCode = $zasilkovnaModel->getCurrencyCode($pricesCurrencyId);  

        $cartCurrencyShipmentPrice =  $method->{'packet_price_' . strtolower($pricesCurrencyCode)};  

        //if using currency that is not set in plugin, convert czk price to used currency
        if(empty($cartCurrencyShipmentPrice)){
          $cartCurrencyShipmentPrice =  $method->{'packet_price_' . strtolower('czk')};   
          $db = JFactory::getDBO (); 
          $q="SELECT virtuemart_currency_id FROM #__virtuemart_currencies WHERE currency_code_3='CZK'";
          $db->setQuery($q);
          $pricesCurrencyId=$db->loadResult();  
        }

        $value = $currency->convertCurrencyTo ($pricesCurrencyId, $cartCurrencyShipmentPrice);//convert hack to keep constant shipment prices. VM2 converts it back later in cart checkout

        /*
          Set free shipping
        */
        $freeShippingTreshold = $method->{'free_shipping_treshold_' . strtolower($pricesCurrencyCode)};
        if(isset($freeShippingTreshold) &&
          $freeShippingTreshold >= 0 && 
          $cart->pricesUnformatted['salesPriceWithDiscount'] >= $freeShippingTreshold){
          $value = 0;
        }
        return $value;    
    }
